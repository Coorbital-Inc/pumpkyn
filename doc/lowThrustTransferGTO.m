...
% This live script demonstrates a **minimum-time, low-thrust transfer**
% of a 15 kg CubeSat (Tmax = 25 mN) from a geostationary transfer orbit
% (GTO-like, Earth-centered) into a **7-petal tulip-shaped orbit**
% in the Earth–Moon Circular Restricted Three-Body Problem (CR3BP).
%
% The target tulip orbit exhibits **zero lunar occultation**, making it
% well-suited for **PNT** (Positioning, Navigation, and Timing) and
% **lunar communications relay architectures**.
%
% ---
% **Sections**
%  1. Define constants and nondimensional scaling
%  2. Set initial GTO state and convert to CR3BP coordinates
%  3. Fetch tulip seed orbit and propagate
%  4. Select matching departure and arrival points
%  5. Define low-thrust spacecraft and solver parameters
%  6. Solve the minimum-time transfer problem
%  7. Propagate and visualize the transfer trajectory
%  8. Compute propellant and ΔV results
%
% ---
% **Dependencies:**  
% All CR3BP routines are part of the `pumpkyn.cr3bp.*` API.  
% Variables ending in `_ND` are nondimensional (CR3BP-scaled).
...
% - If `tfMin` fails to converge:
%    * Adjust the costate guess (`lambda0_tf`)
%    * Use continuation on `Tmax` or final time
%    * Reduce solver tolerances or switch ODE integrator
% - Verify `pumpkyn.cr3bp.tfMin` and `.tfMinProp` signatures
%   match your toolbox version.
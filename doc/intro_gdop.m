...
%
% This tutorial demonstrates how to compute the **Geometric Dilution of Precision (GDOP)**
% for a surface observer located at the lunar south pole, using a
% constellation of satellites in a tulip-shaped orbit family modeled in the
% Earth–Moon CR3BP.
%
% GDOP quantifies how the geometry of the visible satellites affects
% position and timing accuracy for navigation or communication users.
% Lower GDOP values indicate better geometric diversity and higher precision.
...
%% Interpretation:
% Steps indicate discrete changes in which satellites are visible.
% At least four satellites are needed for 3D navigation.
% Periodic gaps reveal coverage limits inherent to the resonance geometry.

%% Compute the GDOP of an Observer:
% Compute the full Dilution of Precision (DOP) metrics.
% pumpkyn.cr3bp.dop() returns a matrix with GDOP, PDOP, HDOP, and VDOP values.
...
%% Interpretation:
% Lower GDOP → better satellite geometry and stronger navigation accuracy.
% Higher GDOP → satellites are clustered or low on the horizon.
% The periodic GDOP oscillations reflect the constellation's resonant motion.
%
% This analysis can help identify optimal orbital geometries and phasing
% for continuous PNT coverage of the lunar south pole.

...
vidName = 'orbit_animation.mp4';           % output filename
v = VideoWriter(vidName, 'MPEG-4');        % use MPEG-4 format (good quality)
...
hL = plot3(rv(1:2,1), rv(1:2,2), rv(1:2,3), 'w','LineWidth',2);
grid on;
axis equal;
set(gca, 'clipping', 'off');
view(90.0, 10);
xlim([-0.1 1-muStar-0.1]);
ylim([-0.2 0.2]);
zlim([-0.2 0.1]);
axis manual;


for tt=1:30:size(rv,1)
    set(hL,'xData',rv(1:tt,1), 'yData',rv(1:tt,2),'zData',rv(1:tt,3));
    drawnow;
    frame = getframe(gcf);
    writeVideo(v, frame);
end

rvTgtShift = circshift(rvTgt,-idx_f,1);

hT = plot3(rvTgtShift(1,1), rvTgtShift(1,2), rvTgtShift(1,3), 'w','LineWidth',2);

for tt=1:20:size(rvTgtShift,1)
    set(hT, 'xData', rvTgtShift(1:tt,1), 'yData', rvTgtShift(1:tt,2),'zData',rvTgtShift(1:tt,3));
    drawnow;
    frame = getframe(gcf);
    writeVideo(v, frame);
end

function [lla] = ECEF2LLA(posvececef,dim3) 
%------------------------------------------------------------------------------- 
% function [lla] = ECEF2LLA(posvececef, dim3) 
% 
% references: 
...
%flatten the multi-dimension input matrix to [N x 2]
[posvececef,Seq] = pumpkyn.util.fDim(posvececef,dim3);
%--------------------------------------------------------------------------
...
longitude = zeros(size(x)); 
 altitude = zeros(size(x)); 
%------------------------------------------------------------------------------- 
...
Seq.postShift(end) = 3;
lla = pumpkyn.util.eDim([latitude,longitude,altitude],Seq);
end

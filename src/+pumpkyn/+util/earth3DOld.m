...
if strcmp(options.type,'day')
    image_file = 'earth-clouds-16k.jpg'; %'BMNG_world.topo.bathy.200405.3.2048x1024.jpg';
...
if ~strcmp(options.type,'BW') && options.AddShading
    ...
    % --- Solid Body (Globe) ---
    globe.FaceLighting = 'gouraud';     % Smooth interpolation instead of visible polygons
    globe.SpecularStrength = 0.05;      % Landmasses aren't uniformly shiny; keep this low
    globe.SpecularExponent = 10;        % Tighter specular highlights (if over ocean)
    globe.DiffuseStrength = 1.0;        % Maximize Lambertian reflection
    globe.AmbientStrength = 0.05;       % Keep ambient low so the dark side stays dark
    globe.BackFaceLighting = 'unlit';   % No light passes through the solid body
    globe.EdgeLighting = 'none';
    
    % --- Atmosphere (Gas Shell) ---
    atmos.FaceLighting = 'gouraud';     % Critical for a smooth gas gradient
    atmos.SpecularStrength = 0.0;       % Gas diffuses light; it does not have specular glint
    atmos.DiffuseStrength = 1.0;        % Maximize light scattering
    atmos.AmbientStrength = 0.2;        % Slightly higher ambient allows a faint halo on the dark side
    atmos.BackFaceLighting = 'lit';     % Crucial: Allows light to bleed around the terminator
    atmos.EdgeLighting = 'none';
    atmos.Annotation.LegendInformation.IconDisplayStyle = 'off';

   % --- Optical Properties ---
    atmos.FaceAlpha = 0.15;             % Keep the gas highly transparent
    atmos.FaceColor = [0.3, 0.6, 1.0];  % Rayleigh scattering blue

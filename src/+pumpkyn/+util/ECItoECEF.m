...
%------------------------------------------------------------------       %
function [r_ECEF v_ECEF a_ECEF,eci_ecef_matrix] = ECItoECEF(JD,r_ECI,v_ECI,a_ECI,dim3)
if nargin == 4
   dim3 = 1; 
end
...
if dim3 > 1
    ...
    [r_ECI,fSeq] = pumpkyn.util.fDim(r_ECI,dim3);
    v_ECI = pumpkyn.util.fDim(v_ECI,dim3);
    a_ECI = pumpkyn.util.fDim(a_ECI,dim3);
    
    %Transpose matricies such that they are 3 x N
    r_ECI = permute(r_ECI,[2 1]);
    v_ECI = permute(v_ECI,[2 1]);
    a_ECI = permute(a_ECI,[2,1]);
end
...
eci_ecef_matrix = T3D(THETA);

r_ECEF = squeeze(MultiDimMatrixMultiply(eci_ecef_matrix,r_ECI));
v_ECEF = squeeze(MultiDimMatrixMultiply(eci_ecef_matrix,v_ECI) + ...
...
if dim3 > 1
    r_ECEF = pumpkyn.util.eDim(permute(r_ECEF,[2 1]),fSeq);
    v_ECEF = pumpkyn.util.eDim(permute(v_ECEF,[2 1]),fSeq);
    a_ECEF = pumpkyn.util.eDim(permute(a_ECEF,[2 1]),fSeq);
end
%----------------------------- End Code-----------------------------------%
